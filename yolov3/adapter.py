...
class AdapterModel:
    ...
    def train(self):
        ...
        if os.path.exists('ObjectDetNet/yolov3/cfg/yolov3-custom.cfg'):
            os.remove('ObjectDetNet/yolov3/cfg/yolov3-custom.cfg')

        cmd_line = 'bash ' + 'ObjectDetNet/yolov3/cfg/create_custom_model.sh ' + str(num_classes)
        os.system(cmd_line)
        data = {
...
def predict(home_path, checkpoint_path):
    ...
    num_classes = sum(1 for line in open(class_names_path))
    if not os.path.exists('ObjectDetNet/yolov3/cfg/yolov3-custom.cfg'):
        cmd_line = 'bash ' + 'ObjectDetNet/yolov3/cfg/create_custom_model.sh ' + str(num_classes)
        os.system(cmd_line)
    data = {
